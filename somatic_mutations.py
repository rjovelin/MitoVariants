...
def PositionsWithCoverage(basecall):
    '''
    (file) -> set
    Take the file with the basecalls and return a set of positions with coverage
    (ie. all positions with mapped reads)
    '''
...
# use this function to remove positions that have no coverage
def RemovePositionWithoutCoverage(Participants_RNA, suffix):
    '''
    (dict, str) -> dict
    ...
    without the positions that do not have any coverage in the mitoseek output for WGS    
    ...
    for ID in Participants_RNA:
        ...
        valid_positions = PositionsWithCoverage(subfolder + 'mito1_basecall.txt')
