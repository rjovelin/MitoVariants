...
def StopCodonPositions(HeteroSummaryFile, MitoGeneFile, Sites = 'mutations', FirstOnly = False):
    ...
    for gene in stops:
        if FirstOnly == False:
            # loop over position, convert to positions relative to start
            ...
            # convert to positions relative to start
            stops[gene] = GenomicPositionToGenePosition(stops[gene], coords[gene][0], coords[gene][1], coords[gene][-1])
    
    # if all positions are recorded and only sites should be recorded, then collapse the PTCs at a given position
    if FirstOnly == False and Sites == 'sites':
...
# use this function to compute the % truncation generated by the 5' most upstream premature stop codon 
def GeneTruncation(HeteroSummaryFile, MitoGeneFile):
    '''
    (file, file) -> dict
    Take the Summary file with heteroplasmies detected by MitoSeek and the file with
    mictochondrial coordinates and return a dictionary with protein-coding gene
    name as key and the percent truncation caused by the 5' most upstream PTC 
    in that gene 
    '''
    ...
    # get the position of the 5' most upstream PTC
    # create dict {gene: [positions]}
    stops = {}
    ...
    # compute the % truncation of each gene
    truncation = {}
    # loop over gene with PTC alleles
    for gene in stops:
        assert type(stops[gene]) == int, 'should be a single position'
        # get the length of the gene
        L = (coords[gene][1] - coords[gene][0]) + 1
        # check gene orientation
        if coords[gene][-1] == '+':
            # truncation = (end - position) / gene_length * 100 
            T = ((coords[gene][1] - stops[gene]) / L) * 100
        elif coords[gene][-1] == '-':
            # truncation = (position - start) / gene_length * 100
            T = ((stops[gene] - coords[gene][0]) / L) * 100
        assert T >= 0, 'truncation should be positive'
        truncation[gene] = T
